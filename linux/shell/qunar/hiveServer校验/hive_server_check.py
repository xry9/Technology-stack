...
mobiles = "18611427353,15210645015,18810019014,18811445785"
...
def testHive(host,port,username,timeout):
    conn = None
    try:
        print "[" + time.strftime("%Y-%m-%d %X",time.localtime()) + "] 访问 %(host)s:%(port)s " % {'host':host,'port':port}
        conn = pyhs2.connect(host=host,port=port,authMechanism="PLAIN",user=username,database='default',timeout=timeout)
        cur = conn.cursor()
        cur.execute("drop table if exists dev_platform_hivesvercheck")
        return "1"
    except Exception, e:
...
def hiveServerCheck(id):
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute(" select host,port,username,password,id,result,timeout,server_type from hive_server_check where valid=1 and id=%s " % id)
        ...
        for data in alldatas:
			...
			if server_type == "hive":
				result1 = testHive(host,port,username,timeout)
            ...
            if result1 != result:
                ...
                conn.commit()
                if result1=="1":
                    message += "%(host)s:%(port)s %(server_type)s 恢复正常;" % {'host':host,'port':port,'server_type':server_type}
                else:
                    message += "%(host)s:%(port)s  %(server_type)s 进入异常状态;" % {'host':host,'port':port,'server_type':server_type}
            elif result1 == "0":
                message += "%(host)s:%(port)s  %(server_type)s 处于异常状态;" % {'host':host,'port':port,'server_type':server_type}
